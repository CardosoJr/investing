...
class Alphavantage:
    ...
    def get_day_data(self, symbol, interval_min):
        query = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol={}&interval={}&apikey={}".format(
            symbol, self.Interval[interval_min], self.Key)
        data = requests.get(query).json()
        result = pd.DataFrame.from_dict(data[f"Time Series ({self.Interval[interval_min]})"], orient = 'index')
        return result
